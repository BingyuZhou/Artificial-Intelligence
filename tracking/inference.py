...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # when the gost is already captured by the pacman, 
        # the Pr of jail location is 1 and other locations are zero.
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        allpossible = util.Counter()
        for pose, p in self.beliefs.items():
            pmodel = self.getPositionDistribution(self.setGhostPosition(gameState, pose))
            for pose_next, p_next in pmodel.items():
                allpossible[pose_next] += p*p_next
        allpossible.normalize()
        self.beliefs = allpossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        repitation = parnum / len(legalPos)
        parlist = []
        parlist = legalPos*repitation
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        parweights = util.Counter()
        
        if noisyDistance is None:
            self.parlist = [self.getJailPosition()]*self.numParticles
        else:
            j = 0
            for particle in self.parlist:
                dis = util.manhattanDistance(particle, pacmanPosition)
                if emissionModel[dis]>0:
                    parweights[j] = emissionModel[dis]
                else:
                    parweights[j] = 0
                j = j+1
                
            #print(parweights)
            if parweights.totalCount() ==0:
                self.initializeUniformly(gameState)
            else:
                # Resampling
                parindex = []
                #for i in range(self.numParticles):
                    #parindex.append(util.sample(parweights))
                
                parindex = util.nSample(parweights.values(), parweights.keys(), self.numParticles)
                
                allpossible = []
                for index in parindex:
                    allpossible.append(self.parlist[index])
                
                self.parlist = allpossible
    ...
    def elapseTime(self, gameState):
        ...
        # Sample a new position for each particle
        for particle in self.parlist:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
            allpossible.append(util.sample(newPosDist))
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        self.beliefs = util.Counter()
        numParticles = len(self.parlist)
        parlist = self.parlist
        
        #print(len(parlist))
        #print(self.numParticles)
        for particle in parlist:
            if particle in self.beliefs.keys():
                continue
            self.beliefs[particle] = float(parlist.count(particle)) / numParticles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # permutation of each ghost's position using cartesian product
        permutation = list(itertools.product(legalPos,repeat = self.numGhosts))
        repitation = self.numParticles / len(permutation)
        self.particles = permutation*repitation
    ...
    def observeState(self, gameState):
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] is None:
                ...
                for particle in self.particles:
                    allpossible.append(self.getParticleWithGhostInJail(particle, i))
                self.particles = allpossible
                numGhostinJail += 1
                if numGhostinJail == self.numGhosts:
                    return
                ...
                for particle in self.particles:
                    ...
                    if emissionModels[i][dis] >0:
                        if flag == 0:
                            parweights[j] = emissionModels[i][dis]
                        else:
                            parweights[j] = parweights[j]*emissionModels[i][dis]
                    else:
                        parweights[j] = 0
                    j = j+1
                flag = 1
        ...
        if parweights.totalCount() == 0:
            ...
            parlist = []
            for index in parindex:
                parlist.append(self.particles[index])
                
            self.particles = parlist    
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            if particle in self.beliefs.keys():
                continue
            self.beliefs[particle] = float(self.particles.count(particle)) / numParticles
        #print(self.beliefs)  
        return self.beliefs
