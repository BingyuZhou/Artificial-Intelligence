...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        value = 0
        if not legalActions: return value
    ...
    def computeActionFromQValues(self, state):
        ...
        legalActions = self.getLegalActions(state)
        action = None
        if not legalActions: return action
        qlist =util.Counter()
        for action in legalActions:
            qlist[(state,action)] = self.getQValue(state, action)
        action = qlist.argMax()[1]
        return action
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        Q = 0
        featureVec = self.featExtractor.getFeatures(state,action)
        for feat,value in featureVec.items():
